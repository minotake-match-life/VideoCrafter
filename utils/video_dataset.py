...
class ShowHowToDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            video_frames (Tensor): Tensor of shape [C, T, H, W] containing video frames. [-1, 1]
            prompts (list): List of prompts corresponding to the video frames. [T, ...]
        """
        ...
        selected_imgs = np.stack([np.array(Image.fromarray(fr).resize((256, 256))) for fr in selected_imgs], axis=0) # 256x256
        video_frames = torch.from_numpy(selected_imgs.copy()).float().div_(255 / 2).sub_(1).permute(3, 0, 1, 2) # [C, T, H, W], [-1, 1]
...
def sequence_collate(batch):
    """
    Collate function for ShowHowToDataset to stack video frames and prompts.
    Args:
        batch (list): List of tuples where each tuple contains video frames and prompts. [B, (C, T, H, W), list of prompts]
    Returns:
        video_frames (Tensor): Stacked tensor of video frames. [B, C, T, H, W]
        prompt (list): List of prompts corresponding to the video frames. [B*T, ...]
    """
